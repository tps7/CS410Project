...
def get_players_from_pbp(play):
    """Given one input gets all of the players out of it in order listed
    """
...
def calc_stats():
    ...
    for i in range(0, len(pbp)):
        ...
        #     continue
            ...
            play = pbp[i]
            if (week == 8 and team[i] == "CHI" and "J.FIELDS" in play):
                print(play)
            yard = yards[i]
            reversed = False
            if "REVERSED" in play:
            ...
            curr_play = ply.Play(week, yard, type, is_td, yard_line_start[i], quater[i], minute[i], second[i], down[i], distance[i], first_down[i])
